...
/**
 * /handshake?token=XXXX
 * Flujo:
 * 1) Recibe token (one-time / exp corto) creado por Crowdlink
 * 2) Valida token con Crowdlink (aquí MOCK)
 * 3) Crea sesión client (customerId + email)
 * 4) Redirige a /onboarding
 *
 * Tu equipo luego reemplaza validateTokenMock() por:
 * - fetch( https://crowdlink.mx/api/burocrowdlink/validate , { token } )
 * - o llamada server-to-server desde un route handler
 */
...
function validateTokenMock(token: string): Promise<ValidateOk | ValidateFail> {
  // Regla mock:
  // - token válido si empieza con "cl_" y mide > 10
  // - devuelve customerId simulado
